...
| `demo.gif` | A 10-second loop. The blocked tab, Back to work returning to the draft, and the popup ending the running session. |
...
The animation samples the interactive demo page eight times per second, not the extension inside real Chrome tabs. The demo boots with a focus session already running and the Headlines tab active, so the real lockscreen is on screen from the first frame, mounted inside that tab's iframe. At three seconds it presses the lockscreen's own Back to work control and verifies that the demo page's fake tab strip switches to the draft, where it types a line. At six seconds it opens the popup, which shows the running session, and at eight seconds it ends the session through the popup's Friction gate, all inside one Chrome tab. `ffmpeg` encodes those frames with a 128-colour palette. Every output must stay below 5 MB.
