...
- [x] The seed matrix covers no activity in local mode, one active hour in Sync mode, and all 24 active hours in local mode. The boundary state includes seven-digit attempt values, a 72-character domain, one completed session, and one session ended early. Development and production use the same frozen timestamp, `2026-09-01T10:00:00.000Z`. Every record stores that timestamp and a per-state seed SHA-256. The production run rejects any difference from the development report.
- [x] The source-module command `STATS_DEV_EVIDENCE_DIR=artifacts/stats-task5/dev NO_COLOR=1 node scripts/capture-stats-dev-evidence.ts` passed with an exact 216-screenshot inventory. It starts an owned strict-port Vite child on port 4178, requires both owned ready output and an HTTP response, and awaits bounded teardown through the existing Task 7 process helper. It writes the report only after the owned page, context, browser, and Vite process close.
- [x] The production command `NO_COLOR=1 npm run build && STATS_EVIDENCE_DIR=artifacts/stats-task5/production STATS_CURATED_IMAGE=docs/images/focus-lock/stats.png NO_COLOR=1 npx playwright test tests/e2e/qa-flows.spec.ts --grep "Task 5 Stats responsive evidence matrix" --reporter=line` passed 1 scenario in 24.2 seconds. It used the freshly built Manifest V3 extension, closed its owned Stats page and browser context before certification, and wrote an independent 216-screenshot inventory. The existing Task 7 production matrix remains an exact 356-record contract.
...
- [x] Development and production reports each contain zero console, page, worker, request, and blocked-request diagnostics. Every screenshot record includes its exact filename, build source, state, theme case, full viewport width and height, scope, byte size, SHA-256, PNG width and height, and seed identity. Validation checks the PNG signature and dimensions, exact disk-set parity, duplicate names, stale names, missing bytes, hashes, and metadata. Each run stages into a unique empty sibling directory. A failed rerun removes the previous success report. A successful run swaps the verified set without mixing old files. Symlinked targets, symlinked approved boundaries, and a post-claim staging swap fail closed.
- [x] `docs/images/focus-lock/stats.png` is a verified full-page 1280 px production capture of the all-hours boundary state in explicit Dark with light operating-system media. It shows current labels, local-only scope copy, tiles, readable charts, the 24-hour heat strip, seven-digit values, and completed or ended-early session outcomes.
- [x] `NO_COLOR=1 npm run check` passed 86 test files with 2,017 tests passed and 2 skipped. Biome, TypeScript, deterministic icon generation, and the production build passed.
- [x] `NO_COLOR=1 npm run e2e` rebuilt the extension and passed all 46 Playwright scenarios in 5.4 minutes. The integrated run included the unchanged 356-record Task 7 matrix and the 216-record Task 5 Stats matrix.
