...
- [x] The development Options matrix reran at 375, 768, and 1280 px in Auto with light media, Auto with dark media, explicit Light with dark media, and explicit Dark with light media. Its 12 run reports contain 84 measured category row-to-state gaps, all exactly 0 px. A fresh isolated dev-browser pass measured the clean save bar as `position: static`, the dirty bar as `position: sticky`, zero clean category intersections, zero dirty target-row intersections, and no horizontal overflow in all 12 cases. Popup Hard lock click and overlay provenance received new full and focused dev-browser captures across all applicable theme and viewport cases. Visual inspection found no new defect.
...
- [x] The expanded production extension matrix contains 332 full-page and focused captures from a fresh isolated build of application-source commit `537ed1a71d7f6ad70ca2c28469bdf94d62cdf969`. Popup ran at 340 px. Options, Privacy, Stats, typed and untyped gates, the removed force-end state, and both overlay forms ran at 375, 768, and 1280 px as applicable. Every surface ran in all four theme and media cases. The matrix covers long-list focus, Flexible hover, Friction focus, Hard lock click, invalid-domain error focus, category-off focus, a partial category, sticky save, a real quota-backed durable Sync error, both Privacy confirmations, unsupported-tab reason, current Stats language, typed and untyped cancellation, force-end removal, overlay provenance, and the stopped-document message. The normal no-environment-variable matrix executed every assertion and passed 1 scenario in 49.2 seconds while writing only to the Playwright per-test output directory.
- [x] The final persistent production test passed 1 scenario in 48.3 seconds with `FOCUS_LOCK_E2E_DIST=<isolated-build> TASK7_EVIDENCE_DIR=artifacts/daily-product-surfaces-task7 NO_COLOR=1 npx playwright test tests/e2e/qa-flows.spec.ts --grep "Task 7 production evidence matrix" --reporter=line`. Evidence mode rejects repository `dist/`, missing provenance, dirty application source, application-source commit or tree mismatches, and before-to-after build mutation. The report records application-source SHA-256 `8c9505da60fd8dfa48ebb4ae8aadb47fa18a9187839035581098ec735d039202`, dist-tree SHA-256 `067280d17ceba7e64d9b1901d35fe98f6d27b8cd962a9ac5cc3f0f77e9f9e815`, and built-manifest SHA-256 `41932cb819617ee6bdaeb0d97ecbeedd6b3ceb315bb2d6832579e4f32f433e86`, unchanged before and after capture. Its context-closed audit contains zero unexpected console, page, worker, request, or blocked-request diagnostics. It classified two browser-shutdown worker messages separately.
...
- [x] The production Options report contains 84 category row-to-state gap samples, all exactly 0 px. In all 12 Options cases, clean save bars were static and intersected no category row or state. Dirty save bars were sticky and intersected no targeted category content.
...
- [x] `artifacts/daily-product-surfaces-task7/evidence-manifest.json` inventories every evidence file except itself, records SHA-256 and byte size for each file, records exact image dimensions and state metadata for each PNG, and cross-checks exact set parity for all 332 expanded production captures against `production-run-report.json`. Duplicate inventory entries and any missing or unexpected production image fail generation. The no-self-hash rule is explicit in the manifest. All 34 Task 7 non-PNG support artifacts pass the project formatter.
- [x] The focused evidence and save-bar unit run passed 11 tests across 2 files. The scoped `qa-flows.spec.ts` and `system.spec.ts` run passed all 11 scenarios in 1.9 minutes against the immutable production build.
- [x] `NO_COLOR=1 npm run check` passed 78 test files with 1,970 tests passed and 2 skipped. Biome, TypeScript, deterministic icon generation, and the production build passed.
- [x] The earlier integrated `NO_COLOR=1 npm run e2e` passed all 44 scenarios in 4.8 minutes. The fresh post-evidence run passed all 44 scenarios in 5.1 minutes. Commit `537ed1a71d7f6ad70ca2c28469bdf94d62cdf969` fixed the paused-session timeout root cause: the engine scheduled the next wake from the paused phase end even when the session wall clock ended first. It now schedules the earlier of phase and session end. A unit regression asserts the exact wake time, and the browser regression verifies paused UI leaves when the wall-clock session end arrives.

Task 7 contains 647 PNG files and 34 support artifacts in `artifacts/daily-product-surfaces-task7/`. The screenshots comprise 306 development captures and 341 production captures, including the original 9 critical production captures and the expanded 332-capture production matrix. Representative evidence:

- Development Hard lock click detail: `task7-dev-popup-<theme-case>-340-hard-click-full.png` and matching `-focused.png` files.
- Development unsupported-tab reason: `task7-dev-popup-<theme-case>-340-unsupported-tab-full.png` and matching `-focused.png` files.
- Development overlay provenance: `task7-dev-overlay-<theme-case>-<375|768|1280>-provenance-full.png` and matching `-focused.png` files.
...
- Current Stats: `task7-<dev|production>-stats-<theme-case>-<375|768|1280>-current-language-<full|focused>.png`.
- Current gate states: `task7-<dev|production>-gate-<theme-case>-<375|768|1280>-<typed-gate|untyped-gate|force-end-removed>-<full|focused>.png`.
- Current stopped-document overlay: `task7-<dev|production>-stopped-overlay-<theme-case>-<375|768|1280>-stopped-document-<full|focused>.png`.
- Machine-readable audit: `evidence-manifest.json`, `production-run-report.json`, the Popup and overlay run reports, the three current-surface dev reports, and the 24 per-case Options and Privacy reports.
