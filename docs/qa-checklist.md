...
- [x] The seed matrix covers no activity in local mode, one active hour in Sync mode, and all 24 active hours in local mode. The boundary state includes seven-digit attempt values, a 72-character domain, one completed session, and one session ended early. A focused unit validates every seed through the production `isStatsBundle` response boundary, including the August-to-September calendar transition.
- [x] The source-module command `STATS_DEV_EVIDENCE_DIR=artifacts/stats-task5/dev NO_COLOR=1 node scripts/capture-stats-dev-evidence.ts` passed with an exact 216-screenshot inventory. It starts an owned strict-port Vite child on port 4178, requires both owned ready output and an HTTP response, and awaits bounded teardown through the existing Task 7 process helper.
- [x] The production command `NO_COLOR=1 npm run build && STATS_EVIDENCE_DIR=artifacts/stats-task5/production STATS_CURATED_IMAGE=docs/images/focus-lock/stats.png NO_COLOR=1 npx playwright test tests/e2e/qa-flows.spec.ts --grep "Task 5 Stats responsive evidence matrix" --reporter=line` passed 1 scenario in 23.6 seconds. It used the freshly built Manifest V3 extension and wrote an independent 216-screenshot inventory. The existing Task 7 production matrix remains an exact 356-record contract.
- [x] Each 216-file inventory covers 3 states, all 4 theme and media combinations, widths 375, 768, and 1280 px, and 6 capture scopes. The scopes are full page, tiles or the empty summary, heat strip, charts, table disclosure, and responsive session records. No focused capture substitutes a hidden or nonexistent component.
- [x] Both 36-case geometry reports record a minimum computed visible chart-text size of 12 CSS px and zero document horizontal overflow. At 375 px, populated states record the wide session table as `display: none`, the mobile articles as visible, and zero article horizontal overflow. At 768 and 1280 px, the desktop session table is visible.
...
- [x] Development and production reports each contain zero console, page, worker, request, and blocked-request diagnostics. Every screenshot record includes its filename, build source, state, theme case, viewport, scope, byte size, and SHA-256.
...
- [x] `NO_COLOR=1 npm run check` passed 85 test files with 2,003 tests passed and 2 skipped. Biome, TypeScript, deterministic icon generation, and the production build passed.
- [x] `NO_COLOR=1 npm run e2e` rebuilt the extension and passed all 46 Playwright scenarios in 5.4 minutes. The integrated run included the unchanged 356-record Task 7 matrix and the new 216-record Task 5 Stats matrix.
