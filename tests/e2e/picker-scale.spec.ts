...
test('ten thousand tab records keep DOM bounded and support search and offscreen keyboard navigation', async ({
  ...
  await startTestSession(extPage, { durationMin: 5 });
...
test('the picker shows cached favicons, recent tabs and labelled return destinations', async ({
  ...
  try {
    ...
    await expect(extPage.getByRole('button', { name: label, exact: true })).toBeEnabled();
    await expect(extPage.locator('.return-work-button')).toContainText('Most recent reference');
    await expect(extPage.locator('.return-work-button')).toContainText('127.0.0.1');
    await node(cdp, label);
    await extPage.getByRole('button', { name: 'End session', exact: true }).click();
    await expect(extPage.getByRole('button', { name: label, exact: true })).toBeEnabled();
    await extPage.getByRole('button', { name: label, exact: true }).click();
    await expect
      .poll(
        (): Promise<boolean> =>
          worker.evaluate(
