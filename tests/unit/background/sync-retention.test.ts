...
import type { DailyAgg, MonthlyAgg } from '../../../src/shared/types';
...
describe('pending Sync retention', () => {
  ...
  it('compacts a 399-day nonempty session before the pending batch reaches Sync', async () => {
    ...
    // The day-by-day aggregates a 399-day session leaves behind, queued but not yet in Sync. The
    // session that produced them is now the controller's to run, so the batch is seeded directly.
    const days: number = 399;
    for (let index: number = 0; index < days; index++) {
      const at: number = T0 + index * DAY_MS;
      const date: string = localDateStr(at);
      writer.queue(
        `agg:${deviceId}:${date}`,
        daily(date, {
          focusMs: DAY_MS,
          sessionsStarted: index === 0 ? 1 : 0,
          pauseMsEarned: index === 0 ? DEFAULT_SETTINGS.pause.capMs : 0,
          unlockMsSpent: index === 0 ? DEFAULT_SETTINGS.pause.unlockMs : 0,
        }),
      );
    }
    // Compaction runs on the last day of the run, so the newest aggregate is today's.
    const now: number = T0 + (days - 1) * DAY_MS;
    ...
    expect(totals).toEqual({
      focusMs: days * DAY_MS + existingFocusMs,
