...
async function open(count: number, icons?: (tabId: number) => Promise<unknown>): Promise<Mock> {
  ...
  showOverlay(
    { blocked: true, reason: 'default', matchedPattern: null },
    { ...emptySnapshot(Date.now()), phase: 'focus', startedAt: 1 },
  );
...
it('makes surrounding controls inert and names the saved destination in the back button', async (): Promise<void> => {
  await open(5);
