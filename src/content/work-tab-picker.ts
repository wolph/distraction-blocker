...
import { type PickerControls, pickerControls, status } from './work-tab-picker-view';
...
export function createWorkTabPicker(
  ...
  const inert: Map<HTMLElement, boolean> = makeSurroundingsInert(trigger);
  const close: (restoreFocus?: boolean) => void = (restoreFocus: boolean = true): void => {
    ...
    index.cancel();
    list.close();
    for (const [node, previous] of inert) {
      node.inert = previous;
      node.toggleAttribute('inert', previous);
    }
    element.remove();
  ...
  const load: () => Promise<void> = async (): Promise<void> => {
    ...
    list.replace([]);
    message('Finding available tabs...');
    body.setAttribute('aria-busy', 'true');
    count.textContent = '';
...
function makeSurroundingsInert(trigger: HTMLElement): Map<HTMLElement, boolean> {
  const previous: Map<HTMLElement, boolean> = new Map();
  for (const child of Array.from(trigger.parentElement?.children ?? [])) {
    if (!(child instanceof HTMLElement)) continue;
    previous.set(child, child.inert === true || child.hasAttribute('inert'));
    child.inert = true;
