...
- [x] Live-session runtime: the runtime of a running session holds the focus intention and the addresses of the pages it is enforcing against, and it lasts as long as that session lasts. A timed session ends on its own. A session set to run until stopped has no timer, so it lasts until the person ends it. Delete local history does not reach this runtime, which is deliberate, because a session cannot keep enforcing if the state it runs on is deleted underneath it. Ending a session clears the session and its intention. The addresses of the pages it enforced against stay in the local runtime until a later session replaces them or all Focus Lock data is deleted.
