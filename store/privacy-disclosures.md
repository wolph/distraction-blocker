...
- [x] Live-session runtime: the runtime of a running session holds the focus intention and the address of each tab the session is blocking, and it lasts as long as that session lasts. A timed session ends on its own. A session set to run until stopped has no timer, so it lasts until the person ends it. Delete local history does not reach this runtime, which is deliberate, because a session cannot keep enforcing if the state it runs on is deleted underneath it. Ending a session clears the session and its intention, and starts a cleanup that sends one clear instruction to each website tab open at the time, whether or not that tab was blocked. Each instruction holds that tab's address while the cleanup runs, and the cleanup removes every stored address when it completes.
- [x] Export: Settings > Privacy and data > Export local event log downloads the detailed local event log as JSON.
- [x] Local deletion: Settings > Privacy and data > Delete local history removes historical full URLs, focus intentions, and detailed session events from the local event log. In local-only mode, it also removes local aggregate statistics. It does not clear the current live-session runtime, which holds the focus intention and the address of each tab the session is blocking. A session set to run until stopped holds that runtime until the person ends it. See Retention for what ending a session clears.
