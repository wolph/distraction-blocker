...
- [x] Live-session runtime: the runtime of a running session holds the focus intention and the address of every website tab open while it runs, and it lasts as long as that session lasts. A timed session ends on its own. A session set to run until stopped has no timer, so it lasts until the person ends it. Delete local history does not reach this runtime, which is deliberate, because a session cannot keep enforcing if the state it runs on is deleted underneath it. Ending a session clears the session and its intention. It leaves one clear instruction for each website tab that was open at the time, each holding that tab's address, whether or not that tab was blocked. Those instructions are what a returning page checks itself against, so they outlive the session by design. The next session replaces them, and no other Focus Lock control removes them on its own.
...
- [x] Delete everything: Settings > Privacy and data > Delete all Focus Lock data is the one control that can delete all Focus Lock data. It removes every Focus Lock record from this device and, when sync is on, the synced copies from Chrome Sync, ends a running session, and returns the extension to setup.
