...
- [x] Live-session runtime: the runtime of a running session holds the focus intention and the address of every website tab open while it runs, and it lasts as long as that session lasts. A timed session ends on its own. A session set to run until stopped has no timer, so it lasts until the person ends it. Delete local history does not reach this runtime, which is deliberate, because a session cannot keep enforcing if the state it runs on is deleted underneath it. Ending a session clears the session and its intention, and starts a cleanup that sends one clear instruction to each website tab open at the time, whether or not that tab was blocked. Each instruction holds that tab's address while the cleanup runs, and the cleanup removes every one of them when it completes.
