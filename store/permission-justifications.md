...
| `favicon` | Shows each eligible tab's site icon in the work tab picker, in the popup before a session starts and in the blocking interface during one. Icons are read from Chrome's local favicon cache through the extension's `_favicon/` endpoint. No request is made to the site. | The picker lists open tabs by title and hostname, and the icon is what lets a person tell two tabs on the same site apart. Chrome only serves its favicon cache to an extension that declares this permission. | Used |
