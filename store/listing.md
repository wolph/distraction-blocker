...
Chrome Sync is a setup choice and can be disabled later in Settings > Privacy and data. The same page can export the local event log and separately delete remote Focus Lock data from Chrome Sync after sync is disabled. It can also delete all Focus Lock data: every Focus Lock record on this device and any Focus Lock copies left in Chrome Sync. That control is available while no session is running, and it returns the extension to setup.
